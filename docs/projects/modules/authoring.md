...
| --- | --- | --- | --- |
| `underlays` | inside `.world`, before the items, in world units | `UnderlayFacts { canvas, drag, readText? }` — `drag` is `{ itemIds, dx, dy } \| null`, the live gesture, so a line can ride it before the replica moves; `readText(hash)` (wireframes phase 5) reads a file of this canvas, hash-cached, for lines computed from what a file SAYS — the wireframe arrows read each kept screen's spec; `selection` (24 Sep 2026) is this viewer's selection, for a transient light that writes nothing — a selected wireframe prototype lights the screens it plays | Draw under the items: a node is chromeless text and a line over it strikes through the words. The mind map's lines. |
...
| `dialogs` **(proposed)** | the shell's Modal, opened by an action or slash command's `opens` id | `DialogFacts { canvasId, groupMode, canvas, selection, args, rcParked, canEdit, host }` | Declares `{ id, title, wide?, component }`. `args` contains the words after the slash command. The host is a `DialogHost`: `WebHost` plus `close()`, and (wireframes phase 5) `readText(hash)` and `getCanvas()` as a workspace host has them, `judge(question)` — the home's `POST /api/judgment`, a typed question answered with the home's key, which never reaches the browser — and `notice(text, problem?)` for what happens after the dialog has closed. Opening is not a write: the dialog may be shown to a reader and must explain its disabled acts. |
