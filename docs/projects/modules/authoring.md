...
| --- | --- | --- | --- |
| `underlays` | inside `.world`, before the items, in world units | `UnderlayFacts { canvas, drag, readText? }` — `drag` is `{ itemIds, dx, dy } \| null`, the live gesture, so a line can ride it before the replica moves; `readText(hash)` (wireframes phase 5) reads a file of this canvas, hash-cached, for lines computed from what a file SAYS — the wireframe arrows read each kept screen's spec | Draw under the items: a node is chromeless text and a line over it strikes through the words. The mind map's lines. |
