...
note: "#78 — the third isomorphic surface. design.md is the argument: the API already exists as the CLI's private middle layer, and the work is a seam. These journeys are the acceptance suite, phases.md the walk — four phases, the seam first, each journey closed by playing it for real. Phase 1 closed 31 Aug: the seam holds, `@isocan/api` owns the Node client, the CLI consumes it."
---
