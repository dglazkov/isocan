...
| [mindmap](mindmap/) | Riffing with an agent into a mind map that is a FILE, and that the rest of the work reads as context. [`design.md`](mindmap/design.md) argues the shape: the map is one item whose blob is a markdown outline, not a graph of items joined by a new op type — this canvas has no edges, and the three things asked for (a file, a riff, something other work can read) each rule that out on their own. | **Researched, 28 Aug 2026. Nothing built.** Stage 1 — the role, the outline, the CLI — is useful before any map view exists, which is the test of whether the walk is honest. |
