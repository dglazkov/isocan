...
| [ui-refresh](ui-refresh/) | The canvas as a stage rather than a workbench with tools bolted to the walls — a floating rail, results drawn beside the message that made them, and chrome that arrives when summoned. [`phases.md`](ui-refresh/phases.md) is the walk; the design itself lives on canvas `prj_trml8m4Zfh` (seven screens, a rationale, and a spec for two of them). | **Phase 0 answered, 28 Aug 2026; nothing built.** Rail on the left, the lane DRAWS and never moves items, and system stays the theme default — so most people meet these surfaces in light, which is the harder frosted case and the one phase 1 looks at first. Both themes and no lost features are gates on every phase. |
