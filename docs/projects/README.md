...
| [auto-upgrade](auto-upgrade/) | A CLI that catches up with its home, in a project that ships several times a day. | Designed 25 Aug 2026. Not chosen. Its step 1 stands alone and is four lines: the hosted home currently **cannot say which build it is** — `ARG ISOCAN_BUILD_SHA` is passed, stored, and read by nobody. |
