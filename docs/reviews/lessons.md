...
| 21 | **A colour that cannot be a token still needs a per-theme correction — and the correction gets written for the theme somebody was looking at.** The value is fixed by something outside the stylesheet (an identity, a brand, a file's own content), so the theme cannot move it; the fix is to move the INK toward the ground instead, and that fix is a decision per theme that reads as a decision per bug. | The identity palette. Seven colours, one literal each, shared by every client and stamped into SVG drawings. Dark had the correction — `color-mix(… 55%, var(--ink))` — and its comment said "light mode leaves it exactly as chosen", which is true and was the bug: on vellum all seven read at **3.11–4.22:1** inside a `.mention-me` chip, whose background is a 24% wash of the very colour written on it, so ink and ground are one hue apart by construction and measuring the colour against the card alone (4.23–6.09) missed it. Shipped for as long as mentions have existed. | `--identity-mix`, a token with a value in both blocks, and `packages/web/test/themes.test.ts`, which derives every ratio from `IDENTITY_COLORS` and the two percentages rather than pasting numbers — so adding a colour to the palette fails there. The guard that matters is the last case: both themes must land at the **same floor** (4.90 and 4.94), not merely both above 4.5. Two themes drift apart one theme at a time, and "both pass" cannot see it. Generally: **when a value is corrected per theme, the guard is the comparison between the themes, not the threshold each clears.** |
| 22 | **A whole file that no test parses.** Every check on it is a regex, which is the right tool for asking what it SAYS and useless for asking whether the runtime can read it at all. | Two on the same file, an hour apart. An edit dropped a comment's opening `/*`; six lines of prose became top-level CSS, postcss refused `styles.css`, and the app served a blank page behind Vite's error overlay — with `npm test` green, 1216 passing, including nine checks that read that exact file. Earlier the same class: three colour literals sat in `Minimap.tsx` as SVG presentation attributes (`fill="#c6c9c0"`, `stroke="#1f3fd0"`), which take no `var()` and so could not follow the theme, while `tokens.test.ts` — the file whose entire job is "no literal colour" — reads only the stylesheet and could never have seen them. | Two checks in `tokens.test.ts`: every `/*` is closed, and braces nest with no `;` at depth 0 (in a file with no `@import`, a top-level `;` is a declaration or a comment that has escaped its wrapper — both are this bug). And `themes.test.ts` extends the no-literals rule to every `.ts`/`.tsx` under `src/`, at the positions where a literal PAINTS. The shape: **ask what would notice if this file became garbage.** If the answer is "the browser, later", the suite has a hole the size of the file — and a guard scoped to one file is scoped to one file, not to the rule it is named for.  |
