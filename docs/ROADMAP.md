...
| **project** | [inception](docs/projects/inception/design.md) | 2026-09-02 | phases 0–1 built 2 Sep — core/canvasitem.ts, the card drawn live from the other canvas's snapshot one level deep, ↗ and double-click open a tab, `isocan canvas place` by ref or address, and the popup from the rail and ⌘K (search or paste an address); phases 2–4 (the screenshot, thumbnails everywhere, another home) designed and not built · see on-demand, standing-agents |
...
| **project** | [roles](docs/projects/roles/journey.md) | 2026-09-02 | journeys, design and six phases written 2 Sep 2026; phases 1 and 2 built and walked the same day — the four-rung ladder compared in core, the creator's floor at the door, the read-only canvas, `own` as a grantable row with every grant write owner-gated, and a rung change that reaches an open socket as `standing` or `withdrawn`; journeys 1 and 2 play, journey 7 for the canvas. Phases 3–6 (the bar, the space, the group, the compatibility walk) are designed and not built. Starts from the 1 Sep roles research (a four-rung ladder on one canvas) and adds the space, the group, and the bar, because the journeys need a scope wider than one canvas and a subject wider than one address · see multiuser, multi-identity |
