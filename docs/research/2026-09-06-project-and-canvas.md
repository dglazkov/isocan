...
**6 September 2026.** Asked as: *make sure we are consistent naming project
...
Across the workspaces' own source (`packages/*/src`, tests excluded) on
6 September:

| Word | Occurrences | Where |
| --- | --- | --- |
| `canvas` | 4,879 | everywhere a person or an agent reads: the CLI's verbs (`isocan canvas list`), the guide (271 mentions), the README (114), every panel and dialog |
| `project` | 331 | almost all of it in three durable spellings and a handful of code names, listed below |

The three module packages added on 4 and 5 September (`mindmap`, `mermaid`,
`documents`, counted apart from the table above) say "canvas" 93 times and
"project" not once: the newest code in the repo does not reach for the word
at all, which is the same finding from the other end.
...
| Where | To | Cost |
| --- | --- | --- |
| `useCanvasStore.project` (web) — the canvas's record beside `canvas` (its items) | `meta`, or `record`; the type is already `Canvas` | one store field, 19 readers |
