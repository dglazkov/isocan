...
14`), and the fan offering "Keep only the latest 14…" past that depth.

This note originally also proposed a live window on the fan — the newest N
cards drawn, the rest a label. It landed elsewhere and better while this
branch sat: `VersionFanOut` draws a card's document only once the card is
on screen (`useOnScreen`), which bounds the same cost by what a person is
actually looking at rather than by a constant, and needs no number to argue
about. The window was dropped rather than merged on top of it.
