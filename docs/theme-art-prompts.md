...
| `ocean2` | **Ships as `ocean`.** Chosen over `ocean`, which tiles perfectly but reads as a woven lattice: the X-crossings sit on a findable grid. `ocean2` reads as sea. |
...
| `ocean` | Not shipped. See `ocean2` above. |
