...
| Day | | What happened |
| --- | --- | --- |
| **[22 Aug](2026-08-22.md)** | 29 commits | Two disks under one engine, and a colour audit. The multiuser build returns with a different architecture — `Store` becomes an interface with `CloudStore` behind it, and identity is designed as a desk of mechanisms. On the app side, every text element measured against its real background in both themes: 28 contrast failures found, 0 left. |
