 ...
 * surfaces.
 */
export declare function ownerOf(project: {
    createdBy: {
        id: string;
    };
}): string;
...
export declare const RUNGS: readonly Capability[];
/** Is what is held at least what is needed — the one comparison the ladder
 * exists to make. */
export declare function atLeast(held: Capability, needed: Capability): boolean;
/** The higher of two rungs: a person's rung on a canvas is the highest of
 * every row that admits them. */
export declare function highest(a: Capability, b: Capability): Capability;
...
export declare function isCapability(word: unknown): word is Capability;
/**
 ...
 */
export declare function narrowed(capability: Capability | undefined): capability is Capability;
...
export declare function capabilityOf(grant: {
 ...
 */
export declare const capabilityWord: Record<"dialog" | "presence", Record<Capability, string>>;
...
export declare const LINK: GrantSubject;
...
export declare function scopeOf(grant: GrantScope): {
    kind: "canvas" | "space";
    id: string;
};
/** Is this a row on a space? */
export declare function isSpaceGrant(grant: GrantScope): grant is {
    spaceId: string;
};
/**
 ...
 */
export declare const GRANTED_BY_HOME = "home";
...
export declare const GRANTED_BY_MIGRATION = "migration";
/**
 ...
 * itself.
 */
export declare function normalizeAttribute(attribute: string): string;
/** The same, promised to be a subject. Written separately so the type says
 * which side of the equality a caller is on. */
export declare function normalizeSubject(subject: GrantSubject): GrantSubject;
/**
 ...
 */
export declare function upsertAttestation(existing: readonly Attestation[] | undefined, attestation: Attestation): Attestation[];
export declare function attestationSatisfying(subject: GrantSubject, attestations: readonly Attestation[]): Attestation | null;
...
export declare function isLive(grant: Grant): boolean;
...
export declare function ownsSpace(space: {
    createdBy: string;
}, actorId: string): boolean;
/** Is this space still standing? */
export declare function isSpaceLive(space: Space): boolean;
...
export declare const SPACE_NAME_MAX = 80;
/**
 ...
 */
export declare function spaceNameRefusal(name: unknown): string | null;
...
export declare function sameSpaceName(a: string, b: string): boolean;
/** `GET` lists the spaces the badge may see; `POST {name}` makes one. All at
 * the home; a replica forwards through its one home. */
export declare const SPACES_ROUTE = "/api/spaces";
/** `DELETE` marks it deleted. */
export declare const spaceRoute: (spaceId: string) => string;
/** `PUT` adds the canvas; `DELETE` removes it. */
export declare const spaceCanvasRoute: (spaceId: string, canvasId: string) => string;
/** The grants routes, scoped to the space: `GET` lists, `POST` creates. */
export declare const spaceGrantsRoute: (spaceId: string) => string;
/** `DELETE` revokes one. */
export declare const spaceGrantRoute: (spaceId: string, grantId: string) => string;
/** The same `DELETE` with `actorId` and `bar=1` on its query, spelled once
 * like `grantRevokeRoute`. */
export declare const spaceGrantRevokeRoute: (spaceId: string, grantId: string, options?: {
    actorId?: string;
    bar?: boolean;
}) => string;
...
export declare const spaceLinkRoute: (spaceId: string) => string;
...
export declare const spaceActingRoute: (route: string, actorId?: string) => string;
 ...
 */
export declare const NOT_ADMITTED = "not-admitted";
/**
 ...
 */
export declare const VIEW_ONLY = "view-only";
/**
