import type { Actor, CanvasContents, Comment, Item } from "./model.js";
...
export interface DesignComparisonResponse {
    schemaVersion: 1;
    kind: "comparison-response";
    id: string;
    requestId: string;
    epoch: number;
    comparison: DesignQuestionSource;
    authority: {
        kind: "human";
    } | {
...
export interface DesignDecisionInput {
    ...
    authority: {
        ...
        kind: "canvas-delegation";
        responseId: string;
        rationale: string;
    } | {
        kind: "external-report";
        externalRequestId: string;
        reportedOutcome: "choice" | "delegation";
        statement: string;
...
export interface DesignDecisionEffect {
    edit: Extract<Operation, {
        type: "item.edit";
    }>;
    threadId: string;
    comment: Comment;
...
export interface DesignRestoreEffect {
    ...
    item: Extract<Operation, {
        type: "item.removeVersion" | "item.restoreVersion";
    }>;
    threadId: string;
    commentId: string;
    expectedComment: Comment | null;
    comment: Comment | null;
}
/** Direct aliases preserve the enumerable public operation vocabulary and the explicit internal inverse. */
export type DesignDecisionOperation = Extract<Operation, {
...
export interface DesignDecisionsResponse {
    comparisons: DesignComparisonState[];
    decisions: DesignDecisionState[];
    unavailable: Array<{
        id: string;
        reason: string;
    }>;
...
export declare const designDecisionsRoute: (canvasId: string) => string;
/** Both preparation clients capture identical current membership and legacy scope order. */
export declare function designDecisionScope(canvas: CanvasContents, item: Item): DesignScopeBasis;
/** Display eligibility follows canonical audience/outcomes; the serialized writer still rechecks actual custody. */
export declare function designComparisonActions(state: DesignComparisonState, actor: {
    id: string;
    kind: "human" | "agent" | "unknown";
}, joined?: ActorJoins): {
    respond: boolean;
    authorities: Array<DesignDecisionInput["authority"]["kind"]>;
};
/** Authored outstanding keys remain unchanged; only effective adoptions of this admitted epoch settle their read projection. */
export declare function effectiveOutstandingDecisionIds(brief: Pick<import("./design-partner.js").DesignBrief, "requestId" | "epoch" | "outstandingDecisionIds">, briefItemId: string, decisions: readonly DesignDecisionState[]): string[];
