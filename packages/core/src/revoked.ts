...
import { isBar, type Grant, type GrantSubject } from "./grants.ts";
 ...
 * operator typed.
 */
export const OPERATOR_REVOKE_ROUTE = "/api/operator/revoke/:target";
...
export interface OperatorRevokeResponse {
  target: { kind: "canvas" | "space"; id: string };
 ...
 */
export function revokedSentence(grant: Pick<Grant, "revokedAt" | "revokedVia" | "revocation">): string | null {
  if (grant.revokedVia !== "operator" || !grant.revocation || !grant.revokedAt) return null;
  return (
    `Turned off by the operator of this home on ${takedownDate(grant.revokedAt)}: ` +
    `${TAKEDOWN_REASONS[grant.revocation.reason]}. Write to ${grant.revocation.by.replace(/^email:/, "")}.`
  );
}
 ...
 */
export function operatorTurnedOff(rows: readonly Grant[]): Grant[] {
  const live = new Set(rows.filter((row) => row.revokedAt === undefined && !isBar(row)).map((row) => row.subject));
  const newest = new Map<GrantSubject, Grant>();
  for (const row of rows) {
    if (row.revokedAt === undefined || isBar(row) || live.has(row.subject)) continue;
    const seen = newest.get(row.subject);
    if (!seen || row.revokedAt > seen.revokedAt!) newest.set(row.subject, row);
  }
  return [...newest.values()].filter((row) => row.revokedVia === "operator");
}
