...
import type { Attestation } from "./badge.ts";
import type { AttestedKind, GrantSubject } from "./grants.ts";
 ...
 */
export const ATTEST_ROUTE = "/api/attest";
 ...
 */
export const BAD_ID_TOKEN = "bad-id-token";
 ...
 */
export const NO_ATTESTER = "no-attester";
 ...
 */
export function grantSubjectOf(who: string): GrantSubject {
  const trimmed = who.trim();
  if (trimmed.startsWith("email:") || trimmed.startsWith("repo:")) return trimmed as GrantSubject;
  if (trimmed.includes("@")) return `email:${trimmed}`;
  if (trimmed.split("/").length === 3) return `repo:${trimmed}`;
  return trimmed as GrantSubject;
}
