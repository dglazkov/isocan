 ...
 * hues.
 */
export function kindFamily(kind: ItemKind): KindFamily {
  if (kind === "other") return "other";
  const at = (ITEM_KINDS as readonly string[]).indexOf(kind);
  if (at === -1) return moduleKinds().some((k) => k.id === kind) ? "made" : "other";
  return at < (ITEM_KINDS as readonly string[]).indexOf("image") ? "made" : "brought";
}
