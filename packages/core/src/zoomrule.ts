 ...
 */
export function holdsAtZoom(onScreen: number, threshold: number, was?: boolean): boolean {
  return onScreen >= (was === false ? threshold * 1.1 : threshold);
}

/*
 * The fourth rule, the galaxy ground's fade, is the same question asked as a
 * ramp — and a ramp cannot flicker, having no edge to sit on. It is
 * `zoomFade` in `zoomfade.ts`, beside this file rather than in it only so
 * that it rides the lazily loaded theme chunk instead of the entry one.
 */
