...
export const OPERATOR_PURGE_ROUTE = "/api/operator/canvases/:id/purge";
 ...
 */
export function replicasHorizon(relaying: number): PurgeHorizon {
  const seen =
    relaying === 0
      ? "none was relaying when it came down"
      : `${relaying} ${relaying === 1 ? "was" : "were"} relaying when it came down`;
  return {
    kind: "replicas",
    days: null,
    sentence:
      `copies on members' machines are theirs — ${seen}, and a replica or an export ` +
      "somebody holds is not this home's to erase. A takedown never reached them either.",
  };
}
 ...
 */
export function purgeNeedsTakedown(canvasId: string): string {
  return (
    `purge erases; take ${canvasId} down first, so the erasure is the second of two ` +
    `deliberate acts. \`isocan operator takedown ${canvasId} --reason …\``
  );
}
