 ...
 *
 * - `"admissible"` — admitted ∪ what a grant would admit. **No longer the
 *   default** (13 Sep): a link grant admits anybody presenting the address, so
 *   this answer lists every link-open canvas a daemon holds to every badge that
 *   asks — measured on isocan.io as 35 canvases from 22 people, to a window
 *   admitted to one. It stays as a question somebody can ask deliberately, and
 *   an old client that sends no parameter now gets the narrow answer plus the
 *   shelf, which is the direction a default should fail in.
