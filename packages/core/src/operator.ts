 ...
 */
export const OPERATOR_PROOF_HEADER = "x-isocan-operator-proof";
 ...
 */
export const OPERATOR_PROOF_WINDOW_MS = 10 * 60 * 1000;

/** `GET /api/operator/canvases/:id` — what the home holds under that id. */
export const OPERATOR_SHOW_ROUTE = "/api/operator/canvases/:id";
/** `GET /api/operator/log` — the ledger, which only the operator reads. */
export const OPERATOR_LOG_ROUTE = "/api/operator/log";
 ...
 */
export const NO_OPERATOR = "no-operator";
 ...
 */
export const NOT_OPERATOR = "not-operator";
 ...
 */
export const PROOF_STALE = "proof-stale";
 ...
 */
export const NO_OPERATOR_PROOF = "no-operator-proof";
 ...
 */
export const PROVE_PATH_PREFIX = "/operator/prove";
 ...
 */
export function encodeHandoff(handoff: OperatorHandoff): string {
 ...
 */
export function decodeHandoff(segment: string): OperatorHandoff | null {
  try {
    const parsed = JSON.parse(unbase64url(segment)) as unknown;
    if (!parsed || typeof parsed !== "object") return null;
    const { to, state, act } = parsed as Record<string, unknown>;
...
export function provePath(handoff: OperatorHandoff): string {
  return `${PROVE_PATH_PREFIX}/${encodeHandoff(handoff)}`;
}

/** The handoff segment in a path this page was loaded at, or null. */
export function proveSegmentIn(pathname: string): string | null {
 ...
 */
export function loopbackRefusal(to: string): string | null {
  let url: URL;
  try {
    url = new URL(to);
  } catch {
  ...
  if (url.protocol !== "http:") {
    return (
      `this page hands a proof to a terminal on this machine and nowhere else, over http on a ` +
      `loopback address. That link asked for ${url.protocol}//${url.host}.`
    );
  }
  ...
  if (!isLoopbackHostname(url.hostname)) {
    return (
      `this page hands a proof to a terminal on this machine and nowhere else. That link asked ` +
      `for ${url.hostname}, which is not a loopback address — ` +
      "so somebody else's machine was being asked to receive your sign-in."
    );
  }
  return null;
}
...
/**
 * A literal loopback host: `127.0.0.0/8` or `[::1]`.
 *
 * Names are excluded on purpose (see `loopbackRefusal`), and the whole
 * `127.0.0.0/8` block is included because that is what the kernel routes to
 * the machine itself — a listener on `127.0.0.2` is as local as one on
 * `127.0.0.1`, and pretending otherwise would be a rule that looks stricter
 * and is only narrower.
 */
...
function unbase64url(segment: string): string {
