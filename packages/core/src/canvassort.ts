...
export const CANVAS_SORT_LABEL: Record<CanvasSort, string> = {
 ...
 */
export function sortCanvases(canvases: readonly Canvas[], sort: CanvasSort): Canvas[] {
  const by = [...canvases];
  switch (sort) {
    case "name":
      return by.sort(
 ...
 */
export function filterCanvases(canvases: readonly Canvas[], query: string): Canvas[] {
