...
import { DEFAULT_COMMAND_CATALOGUE } from "./command-catalogue.ts";
import type { SlashCommand } from "./commands.ts";

// Agent instructions live outside the menu/parser graph; metadata has one source in the catalogue.
function builtInCommands(): SlashCommand[] {
  const bodies = {
...
/** The complete commands isocan ships with, including the instructions an agent follows.
 * Pure construction lets menu-only consumers leave these bodies in the deferred full registry. */
export const DEFAULT_COMMANDS: SlashCommand[] = /* @__PURE__ */ builtInCommands();
