...
import type { Placement, Operation } from "./ops.ts";
import type { DesignArtifactRef, DesignBrief, DesignQuestionSource, DesignReceipt } from "./design-partner.ts";
import type { QuestionnaireState } from "./questionnaire.ts";
import type { DesignRecordMarker } from "./design-record.ts";
export { parseDesignRequestAction, parseDesignRequestOperation, designIntentHash } from "./design-request-parse.ts";
...
export interface DesignAcceptedResponse { question: DesignQuestionSource; responseId: string }
/** Field provenance distinguishes actual edits and canvas answers from an agent's native-harness report. */
interface DesignFactProvenance { field: string; actorId: string; kind: "direct" | "reported" | "questionnaire"; responseId?: string }
...
export interface DesignContinuation {
  sourceCapture: { bodyHash: string; boundaryCommentId: string } | null;
  scopeCapture: { kind: "source-comment" | "current-selection" | "current-ambient"; revision: number };
...
export interface DesignDiscovery {
  ...
  factBindings: Array<{ questionId: string; factId: string }>;
}
interface RecordPlacement { placement?: Placement; width?: number; height?: number; title?: string }
interface RequestBasis { brief: DesignArtifactRef; epoch: number; versionId: string }
interface RequestChanges { patch?: Partial<DesignBriefFields>; acceptedResponses?: DesignAcceptedResponse[] }
...
export type DesignRequestAction =
  | ({ kind: "start"; requestId: string; itemId: string; versionId: string; source: DesignBrief["source"]; fields: DesignBriefFields; admission: "explicit" | "automatic"; contextRequest?: ContextRequest } & RecordPlacement)
  | ({ kind: "update" | "complete" } & RequestBasis & RequestChanges)
  | ({ kind: "resume"; reason: string; contextRequest?: ContextRequest } & RequestBasis & RequestChanges)
  | ({ kind: "cancel"; reason?: string } & RequestBasis);
/** A receipt's publication carries attributed check data; the daemon does not attest browser execution. */
export interface DesignReceiptPublication extends RecordPlacement { itemId: string; versionId: string; receipt: DesignReceipt }
/** Narrow aliases derive from the directly enumerable canonical operation vocabulary. */
export type DesignRecordOperation = Extract<Operation, { type: "design.request" | "design.receipt" }>;
/** Saved evidence remains readable when changed inputs make it stale or unavailable. */
export interface DesignReceiptState { ref: DesignArtifactRef; receipt: DesignReceipt; author: Actor; marker: DesignRecordMarker; status: "current" | "stale" | "unavailable"; reasons: string[]; checkFreshness: Array<{ checkId: string; status: "current" | "stale" | "unavailable"; reasons: string[] }> }
