 ...
 */
export function undoneSeqs(entries: readonly LogEntry[]): Map<number, number> {
  const undoneBy = new Map<number, number>();
  for (const entry of entries) {
    // Order matters and it is log order: an op can be undone, redone and
    // undone again, and only the last word counts.
