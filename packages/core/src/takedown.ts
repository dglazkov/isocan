...
export function noticeOf(row: CanvasTakedown): TakedownNotice {
  return {
    canvasId: row.canvasId,
    at: row.at,
    reason: row.reason,
    by: addressOf(row.by),
    sentence: takedownSentence(row),
  };
}
 ...
 */
export function takedownSentence(row: Pick<CanvasTakedown, "at" | "reason" | "by">): string {
  return (
    `This canvas was taken down by the operator of this home on ${takedownDate(row.at)}: ` +
    `${TAKEDOWN_REASONS[row.reason]}. Write to ${addressOf(row.by)}.`
  );
}
 ...
 */
export function takedownDate(iso: string): string {
...
export function takedownDateShort(iso: string): string {
  const at = new Date(iso);
  if (Number.isNaN(at.getTime())) return iso;
  return `${at.getUTCDate()} ${MONTHS[at.getUTCMonth()]!.slice(0, 3)}`;
}
...
/** `email:olu@example.com` → `olu@example.com`. The sentence is read by
 * somebody who is about to write an email, so the normalized shape the desk
 * stores is not the shape it goes out in. */
function addressOf(attribute: string): string {
  return attribute.replace(/^email:/, "");
}

// ---- what the operator's two verbs send and are answered ----

/** `POST /api/operator/canvases/:id/takedown`. */
export const OPERATOR_TAKEDOWN_ROUTE = "/api/operator/canvases/:id/takedown";
/** `POST /api/operator/canvases/:id/look`. */
export const OPERATOR_LOOK_ROUTE = "/api/operator/canvases/:id/look";
/**
 ...
 */
export const TAKEDOWNS_ROUTE = "/api/takedowns";
...
export const TAKEDOWNS_CANVAS_PARAM = "canvas";
 ...
 */
export const OPERATOR_LOOK_MS = 60 * 60 * 1000;
 ...
 */
export function operatorLookUrl(home: string, canvasId: string, token: string): string {
