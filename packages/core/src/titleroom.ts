...
export function titleRoom(
  ...
  // The band the name occupies: a strip sitting on top of the item.
  const top = item.y - strip;
  const bottom = item.y;
  let limit = Infinity;

  for (const other of others) {
    // Its own title row makes it taller, upwards, when it is showing one.
    const otherTop = other.titled ? other.y - strip : other.y;
    const otherBottom = other.y + other.height;
    // Bands that do not share any vertical range cannot collide, whatever
    // their horizontal positions.
    if (otherBottom <= top || otherTop >= bottom) continue;
    // Only things to the RIGHT can limit a name growing rightwards. An item
    // overlapping our left edge is already behind the name's start.
    if (other.x <= item.x) continue;
    limit = Math.min(limit, other.x - gap);
  }

  if (limit === Infinity) return Infinity;
  // Never narrower than the card: hovering must not shrink a label.
