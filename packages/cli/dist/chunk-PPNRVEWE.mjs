...
async function readCanvasDesignAudit(io, options) {
  ...
  signal?.throwIfAborted();
  const { auditScreen, DESIGN_AUDIT_VERSION, offSystemTotal } = await import("./designaudit-PQCOFJ76.mjs");
...
async function auditDesignSource(text, designText, options) {
  const { auditScreen, DESIGN_AUDIT_VERSION } = await import("./designaudit-PQCOFJ76.mjs");
...
async function readDesignSourceAudit(io, options) {
  const { DESIGN_AUDIT_VERSION } = await import("./designaudit-PQCOFJ76.mjs");
...
async function repairDesignScreen(io, request) {
  const { prepareDesignRepair, submitDesignRepair, parseDesignRepairBasis } = await import("./design-repair-reader-NQRNVUVN.mjs");
  const { DESIGN_AUDIT_VERSION } = await import("./designaudit-PQCOFJ76.mjs");
