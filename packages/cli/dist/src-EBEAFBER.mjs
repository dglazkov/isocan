...
var canvasArg = {
  session: external_exports.string().trim().min(1).max(256).optional().describe("An explicitly claimed caller session key. Omit to use the machine's ambient identity. Never inferred from clientInfo."),
  canvas: external_exports.string().optional().describe(
    "Canvas id or a unique title prefix. Omit for the canvas bound to the directory the server was started in."
  )
};
async function canvasRead(handle, group, recursive) {
...
function createServer(deps) {
  ...
  const identityOf = (session) => session === void 0 ? void 0 : { session, harness: "mcp" };
  const homeOf = async (session, intent = "read", signal) => {
    const home = await deps.home(identityOf(session), signal);
    signal?.throwIfAborted();
    return home.withSourcePolicy(session === void 0 ? { mode: "exclude" } : { mode: "direct", actorId: home.actor.id, intent }, signal);
  };
  const canvasOf = async (ref, session, intent = "read", signal) => (await homeOf(session, intent, signal)).canvas(ref);
  server.registerTool(
    "list_canvases",
    {
      title: "List canvases",
      description: "The canvases discoverable to this caller, with id and title. Start here when you do not know which canvas the work is on.",
      inputSchema: { session: canvasArg.session }
    },
    async ({ session }, extra) => answering(async () => {
  ...
  server.registerTool(
    ...
    {
      title: "Read a canvas",
      description: "What is on a canvas: every item with its id, title, kind and position. The map, not the contents \u2014 use read_item for one item's text.",
      inputSchema: { ...canvasArg, in: external_exports.string().optional().describe("Read direct members of this canvas group (or legacy sheet)."), recursive: external_exports.boolean().optional().describe("Include nested descendants of the named group.") }
    },
    async ({ canvas, session, in: group, recursive }, extra) => answering(async () => {
  ...
  server.registerTool("read_context", {
    ...
    inputSchema: { ...canvasArg, roots: external_exports.array(external_exports.string()).optional(), in: external_exports.string().optional(), includeExcluded: external_exports.boolean().optional(), thread: external_exports.string().optional(), comment: external_exports.string().optional() }
  }, async ({ canvas, session, roots, in: group, includeExcluded, thread, comment }, extra) => answering(async () => {
    const handle = await canvasOf(canvas, session, "read", extra.signal);
    if (thread !== void 0 || comment !== void 0) {
      if (!thread || !comment) throw new Error("frozen context needs both thread and comment");
      if (roots !== void 0 || group !== void 0 || includeExcluded !== void 0) throw new Error("a saved request already fixes its roots and exclusion policy");
  ...
  server.registerTool("read_context_content", {
    ...
    annotations: { readOnlyHint: true },
    inputSchema: { ...canvasArg, thread: external_exports.string(), comment: external_exports.string(), item: external_exports.string(), face: external_exports.enum(["source", "visual"]).optional(), offset: external_exports.number().int().min(0).optional(), limit: external_exports.number().int().min(1).max(262144).optional() }
  ...
  server.registerTool(
    ...
    {
      title: "Read an item",
      description: "One item's own content \u2014 the text of a note, the source of a document. Ids come from read_canvas.",
      inputSchema: { ...canvasArg, item: external_exports.string().describe("The item id (itm_\u2026).") }
    },
    async ({ canvas, session, item }, extra) => answering(async () => {
      const handle = await canvasOf(canvas, session, "read", extra.signal);
  ...
  server.registerTool(
    "read_threads",
    {
      ...
      description: "The comment threads on a canvas \u2014 what people and agents have said, and what is still unanswered. This is where the reasoning lives; the items are only what it produced.",
      inputSchema: canvasArg
    },
    async ({ canvas, session }, extra) => answering(async () => {
      const handle = await canvasOf(canvas, session, "read", extra.signal);
  ...
  server.registerTool(
    "read_activity",
    {
      ...
      description: "Recent activity on a canvas, newest first \u2014 who did what. Read this before starting work, to find out what happened while you were not here.",
      inputSchema: {
        ...canvasArg,
        limit: external_exports.number().int().min(1).max(100).optional().describe("How many entries (default 10).")
      }
    },
    async ({ canvas, session, limit }, extra) => answering(async () => {
      const handle = await canvasOf(canvas, session, "read", extra.signal);
  ...
  server.registerTool(
    "who",
    {
      ...
      description: "The people and agents this canvas knows, and how to address them. Names here are what an @mention resolves against.",
      inputSchema: canvasArg
    },
    async ({ canvas, session }, extra) => answering(async () => {
      const handle = await canvasOf(canvas, session, "read", extra.signal);
  ...
  const summary = async (handle, personal = false) => ({ layers: await handle.contextSummary(
  ...
  server.registerTool("read_context_summary", {
    ...
    annotations: { readOnlyHint: true },
    inputSchema: canvasArg
  }, async ({ canvas, session }, extra) => answering(async () => summary(await canvasOf(canvas, session, "read", extra.signal), session !== void 0)));
  server.registerTool("read_personal_context", {
    ...
    inputSchema: {
      ...
      item: external_exports.string().trim().min(1).describe("The concrete personal link item id on the destination canvas, not an item inside the private source."),
      cursor: external_exports.string().optional().describe("The nextCursor returned by the previous page of this current contribution set."),
      limit: external_exports.number().int().min(1).max(64).optional().describe("Maximum pieces per page (1\u201364, default 16); this is not a byte limit.")
    }
  }, async ({ canvas, session, item, cursor, limit }, extra) => answering(async () => {
    const handle = await canvasOf(canvas, session, "read", extra.signal);
    extra.signal.throwIfAborted();
    return handle.ctx.client.readPersonal(handle.id, {
      actorId: handle.ctx.actor.id,
      itemId: item,
      mode: "content",
  ...
  server.registerTool("claim_agent", {
    ...
    inputSchema: { session: canvasArg.session.unwrap(), name: external_exports.string().trim().min(1).max(100) }
  ...
  const content = external_exports.string().max(1048576).describe("Text/source content for this item, at most 1,048,576 characters.");
  const contextArgs = { roots: external_exports.array(external_exports.string()).optional(), in: external_exports.string().optional(), includeExcluded: external_exports.boolean().optional() };
  server.registerTool("create_item", {
    ...
    inputSchema: { ...canvasArg, content, mime: external_exports.string().min(1).default("text/markdown"), title: external_exports.string().optional(), in: external_exports.string().optional() }
  }, async ({ canvas, session, content: content2, mime, title, in: group }, extra) => answering(async () => (await canvasOf(canvas, session, "edit", extra.signal)).add({ content: content2, mime, ...title === void 0 ? {} : { title }, ...group === void 0 ? {} : { in: group } })));
  server.registerTool("edit_item", {
    ...
    inputSchema: { ...canvasArg, item: external_exports.string(), content, mime: external_exports.string().optional() }
  }, async ({ canvas, session, item, content: content2, mime }, extra) => answering(async () => (await canvasOf(canvas, session, "edit", extra.signal)).edit(item, { content: content2, ...mime === void 0 ? {} : { mime } })));
  server.registerTool("post_comment", {
    ...
    inputSchema: { ...canvasArg, ...contextArgs, item: external_exports.string().optional(), message: external_exports.string().min(1).max(65536) }
  ...
  server.registerTool("reply_comment", {
    ...
    inputSchema: { ...canvasArg, ...contextArgs, thread: external_exports.string(), message: external_exports.string().min(1).max(65536) }
  ...
  server.registerTool("wait_for_feedback", {
    ...
    annotations: { readOnlyHint: true },
    inputSchema: { ...canvasArg, cursor: external_exports.number().int().nonnegative().optional(), timeoutMs: external_exports.number().int().min(1).max(6e4).default(3e4) }
  ...
  }, { ...cursor === void 0 ? {} : { since: cursor }, timeoutMs, signal: extra.signal })));
  for (const kind of ["canvas", "context"]) {
    const uriFor = (id) => `isocan://canvas/${encodeURIComponent(id)}${kind === "context" ? "/context" : ""}`;
    server.registerResource(kind, new ResourceTemplate(`isocan://canvas/{id}${kind === "context" ? "/context" : ""}`, {
      list: async (extra) => ({ resources: (await (await homeOf(void 0, "read", extra.signal)).ctx.client.listCanvases()).map((canvas) => ({ uri: uriFor(canvas.id), name: `${canvas.title} \u2014 ${kind}`, mimeType: "application/json" })) })
      ...
      const handle = await canvasOf(id, void 0, "read", extra.signal);
