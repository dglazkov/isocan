...
__export(src_exports, {
  ...
  DOCKET_CLAIM: () => DOCKET_CLAIM,
  DOCKET_MARKS: () => DOCKET_MARKS,
  ...
  JUDGMENT_BAD_REQUEST: () => JUDGMENT_BAD_REQUEST,
  JUDGMENT_MAX_BYTES: () => JUDGMENT_MAX_BYTES,
  JUDGMENT_PER_MINUTE: () => JUDGMENT_PER_MINUTE,
  JUDGMENT_RATE_LIMITED: () => JUDGMENT_RATE_LIMITED,
  JUDGMENT_ROUTE: () => JUDGMENT_ROUTE,
  JUDGMENT_TOO_LARGE: () => JUDGMENT_TOO_LARGE,
  JUDGMENT_UNAVAILABLE: () => JUDGMENT_UNAVAILABLE,
  JUDGMENT_UPSTREAM: () => JUDGMENT_UPSTREAM,
  ...
  TEXT_HEADING_EM: () => TEXT_HEADING_EM,
  TEXT_HEADING_LINE: () => TEXT_HEADING_LINE,
  TEXT_KIND: () => TEXT_KIND,
  ...
  docketAnswer: () => docketAnswer,
  docketSlug: () => docketSlug,
  docketVerdict: () => docketVerdict,
  ...
  markPatch: () => markPatch,
  markShortcuts: () => markShortcuts,
  markdownResource: () => markdownResource,
  markdownTargetOffCanvas: () => markdownTargetOffCanvas,
  ...
  moduleMarkIntent: () => moduleMarkIntent,
  moduleMarkPatch: () => moduleMarkPatch,
  ...
  textNodeFit: () => textNodeFit,
  textNodeRefit: () => textNodeRefit,
  textRefit: () => textRefit,
...
// packages/core/src/zoomrule.ts
function holdsAtZoom(onScreen, threshold, was) {
  return onScreen >= (was === false ? threshold * 1.1 : threshold);
}
...
function textIsLegible(worldSize, scale, was) {
...
var SLACK = 1.1;
var LIST_INDENT = 40;
var TEXT_HEADING_EM = 1.5;
var TEXT_HEADING_LINE = /^\s{0,3}#{1,6}\s+/m;
function glyphEm(ch, face) {
  if (face === "mono") return 0.6;
  return (face === "serif" ? 1.06 : 1) * (ch === " " ? 0.28 : /[iljtfI!.,;:'|]/.test(ch) ? 0.3 : /[mwMW@%]/.test(ch) ? 0.9 : /[A-Z]/.test(ch) ? 0.7 : /[0-9]/.test(ch) ? 0.56 : /[a-z]/.test(ch) ? 0.55 : 0.6);
}
...
function textRefit(current2, body, style, face) {
  const need = textBox(body, style, face);
  const width = Math.max(current2.width, need.width);
  const height = Math.max(current2.height, need.height);
  return width === current2.width && height === current2.height ? null : { width, height };
}
function textNodeRefit(item, body, patch = {}) {
...
function textNodeFit(item, body) {
...
function reduceOperation(state, envelope) {
  ...
  switch (op.type) {
    ...
    case "comment.restore": {
      ...
      comments.splice(op.index === void 0 ? comments.length : Math.max(0, Math.min(op.index, comments.length)), 0, op.comment);
...
var roundsMemo = /* @__PURE__ */ new WeakMap();
...
function moduleMarks() {
  return modules().flatMap((m) => m.marks ?? []);
}
function spotlit(item) {
  return modules().some((m) => m.spotlights?.some((p) => item.properties?.[p] !== void 0));
}
...
function reactOp(item, emoji, actorId) {
...
// packages/core/src/docket.ts
var DOCKET_PROP = "docket";
var DOCKET_MARKS = { accepted: "\u2705", rejected: "\u274C" };
var DOCKET_CLAIM = "\u270B";
function docketSlug(item) {
  return item.properties?.[DOCKET_PROP] || null;
}
function docketVerdict(item) {
  const worn = Object.keys(DOCKET_MARKS).filter(
    (v) => (item.reactions?.[DOCKET_MARKS[v]] ?? []).length > 0
  );
  return worn.length === 2 ? "contested" : worn[0] ?? null;
}
function docketAnswer(item, verdict, actorId) {
  const offs = [];
  const ons = [];
  for (const v of Object.keys(DOCKET_MARKS)) {
...
var namesMemo = /* @__PURE__ */ new WeakMap();
...
// packages/core/src/zoomfade.ts
function zoomFade(scale, gone, full) {
...
var SHORTCUTS = [
  ...
  { keys: ["\u2190", "\u2192", "\u2191", "\u2193", "PgUp", "PgDn"], does: "Flip through the slides", group: "Moving around", note: "Full screen only. Items marked \u{1F3AC} (right-click \u2192 Make this a slide) are the deck, in reading order; with none marked, every item is. Page Up/Down flip too, so a presenter's clicker works" },
  { keys: ["N"], does: "Speaker notes, in full screen", group: "Moving around" },
...
// packages/core/src/shortcuttext.ts
...
function checkDesign(doc) {
  ...
  if (tokens.surface !== void 0 && !DESIGN_SURFACES.includes(String(tokens.surface).trim().toLowerCase())) {
    findings.push({
      severity: "warning",
      where: "surface",
...
var JUDGMENT_PER_MINUTE = 60;
var JUDGMENT_UNAVAILABLE = "judgment-unavailable";
var JUDGMENT_TOO_LARGE = "judgment-too-large";
var JUDGMENT_RATE_LIMITED = "judgment-rate-limited";
var JUDGMENT_BAD_REQUEST = "judgment-bad-request";
var JUDGMENT_UPSTREAM = "judgment-upstream";

// packages/core/src/modulemarks.ts
function markOffered(mark, item) {
...
function moduleMarkIntent(items, property) {
  const has2 = (item) => Boolean(item.properties?.[property]);
  const on = !(items.length > 0 && items.every(has2));
  return { on, changing: items.filter((item) => has2(item) !== on) };
}
function moduleMarkPatch(property, on, who) {
...
function convergeOps(plan2) {
  return [
    { type: "item.addVersion", itemId: plan2.parentId, version: plan2.version },
    ...plan2.trash.map((itemId) => ({ type: "item.delete", itemId }))
  ];
}
...
function readIdle(text3) {
  const m = /^(machine|canvas)\s+([0-9]+)\s*m(?:in(?:utes?)?)?$/.exec((text3 ?? "").trim());
  if (!m) return null;
  return { scope: m[1], minutes: Number(m[2]) };
}
function readBudget(lines) {
  ...
  return out.usdPerRun !== void 0 || out.turnsPerRun !== void 0 ? out : null;
}
...
function parsePersona(text3, filename) {
  ...
  const escalate = one2("escalate")?.replace(/^to\s+/, "").trim();
  return {
    name: one2("name") ?? stem,
...
function personaWarnings(persona) {
  ...
  if (persona.escalate && !persona.budget) {
    out.push(
      `hands off to ${persona.escalate} but declares no budget \u2014 machinery never runs a model without one, so there is no small pass to hand off from`
    );
  }
  return out;
}
function escalatedTo(page) {
...
function rollDue(facts2) {
  const { last, seen, now } = facts2;
...
// packages/core/src/slidewrites.ts
...
function markdownTargetOffCanvas(raw) {
