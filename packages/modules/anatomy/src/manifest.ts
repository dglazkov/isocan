...
/** The light half — mimes, kinds, and predicates over a canvas — is its own
 *  module so the entry chunk can have it without the rest. Re-exported here
 *  so nothing that already imports `manifest.ts` learns a second name. */
...
export const anatomyModule: CoreModule = {
  ...
  commands: [{ ...ANATOMY_COMMAND_METADATA, body: ANATOMY_COMMAND_BODY }],
  kinds: ANATOMY_KINDS,
  edges: (canvas) =>
