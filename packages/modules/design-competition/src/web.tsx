...
import { competitionActivation } from "./activation.ts";
 ...
 */
function FighterPicker({ canvasId, groupMode, canvas, selection, args, rcParked, canEdit, host }: DialogFacts) {
        ...
        <span className="dc-note">
          {error ??
            ...
            (!canEdit
              ...
              : rcParked
                ? `${chosen.length || "No"} agent${chosen.length === 1 ? "" : "s"}, ${minutes} minutes, on your own rc and harness. Token cost has not been measured; the rc's turn limits apply.`
...
function BoutTray({ canvas, host }: OverlayFacts) {
  ...
  const take = () => {
    ...
    return host.send([{ type: "item.addVersion", itemId: target.id, version: { id: newVersionId(), blobHash: v.blobHash, size: v.size, mimeType: v.mimeType, filename: v.filename, ...(v.visual ? { visual: v.visual } : {}) } }]);
  };
...
> = {
  ...
  actions: competitionActivation.actions,
  dialogs: [{ ...competitionActivation.dialogs[0]!, component: FighterPicker }],
