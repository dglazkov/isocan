...
export const BLOCKS: Component[] = [
  // ---- navigation
  {
    ...
    draw: ({ props, label, wide }) => {
      const links = wide || str(props, "mobile") === "links"
        ? `<span class="links">${rowsOf(num(props, "links"), (i) => `<span class="lnk">${label(`link-${i + 1}`)}</span>`)}</span>`
        : "";
      return `<div class="navbar"><span class="brand">${img("1/1", "sm")}</span>${links}<span class="sp"></span>${flag(props, "search") ? ibtn("search", "Search") : ""}${
        rowsOf(num(props, "cta"), (i) => btn(label(`cta-${i + 1}`), i === 0 ? "primary" : "secondary", "s"))}${!wide && str(props, "mobile") === "hamburger" ? ibtn("menu", "Menu") : ""}</div>`;
    },
  },
  // ---- layout
  {
  ...
  },
  {
    ...
    draw: ({ props, label, title }) => `<div class="pagehead">${flag(props, "breadcrumbs") ? `<div class="crumbs">${bar(12, "in")}<span>/</span>${bar(12, "in")}<span>/</span>${bar(14, "in")}</div>` : ""}<div class="ph-row"><div class="h h1">${title}</div><span class="sp"></span>${
      rowsOf(num(props, "actions"), (i) => btn(label(`action-${i + 1}`), i === 0 ? "primary" : "secondary", "s"))}</div>${flag(props, "meta") ? bar(35, "meta") : ""}${
  ...
  // ---- auth
  {
    ...
    draw: ({ props, label }) => `<div class="form">${field("Email")}${field("Password", 1)}${
      flag(props, "remember") || flag(props, "forgot") ? `<div class="between">${flag(props, "remember") ? check(label("remember")) : "<span></span>"}${flag(props, "forgot") ? `<span class="lnk">${label("forgot")}</span>` : ""}</div>` : ""}<div class="actions">${btn(label("submit"), "primary", "block")}</div>${
      num(props, "social") > 0 ? `<div class="or">${bar(100, "rule")}</div><div class="actions stack">${rowsOf(num(props, "social"), (i) => btn(`<span class="logo-dot"></span>${label(`social-${i + 1}`)}`, "secondary", "block"))}</div>` : ""}${
      flag(props, "signup-link") ? `<div class="link-row">${bar(28, "in")}<span class="lnk">${label("signup-link")}</span></div>` : ""}</div>`,
  },
  {
    ...
    draw: ({ props, label }) => `<div class="form">${rowsOf(num(props, "fields"), (i) => field(i === 0 ? null : FIELD_KINDS[i - 1] ?? null, i))}${
      flag(props, "terms") ? `<div class="chk"><span class="cb"></span>${bar(30, "in")}<span class="lnk">${label("terms")}</span></div>` : ""}<div class="actions">${btn(label("submit"), "primary", "block")}</div>${
      num(props, "social") > 0 ? `<div class="actions stack">${rowsOf(num(props, "social"), (i) => btn(`<span class="logo-dot"></span>${label(`social-${i + 1}`)}`, "secondary", "block"))}</div>` : ""}${
      flag(props, "signin-link") ? `<div class="link-row">${bar(28, "in")}<span class="lnk">${label("signin-link")}</span></div>` : ""}</div>`,
  },
  {
    ...
    draw: ({ props, label }) => `<div class="form center">${bars(2)}<div class="code">${rowsOf(num(props, "digits"), () => "<span></span>")}</div><div class="actions">${btn(label("submit"), "primary", "block")}</div>${
      flag(props, "resend") ? `<div class="link-row">${bar(24, "in")}<span class="lnk">${label("resend")}</span></div>` : ""}</div>`,
  },
  {
    ...
    draw: ({ props, label }) => str(props, "step") === "request"
      ? `<div class="form">${bars(2)}${field("Email")}<div class="actions">${btn(label("submit"), "primary", "block")}</div><div class="link-row"><span class="lnk">${label("back")}</span></div></div>`
      : `<div class="form center"><div class="glyph">✉</div>${bars(2)}<div class="actions">${btn(label("back"), "secondary", "block")}</div></div>`,
  },
  // ---- onboarding
  {
    ...
    draw: ({ props, label }) => `<div class="onb">${flag(props, "skip") ? `<div class="right"><span class="lnk">${label("skip")}</span></div>` : ""}${
      ...
      rowsOf(num(props, "steps"), (i) => `<i class="${i + 1 === sel(props, "current", "steps") ? "on" : ""}"></i>`)}</div><div class="actions">${btn(label("next"), "primary", "block")}</div></div>`,
  },
  // ---- input
  {
    ...
    draw: ({ props, label }) => `<div class="wizard">${stepsRow(num(props, "steps"), sel(props, "current", "steps"), true)}${
      flag(props, "summary") ? `<div class="card">${bars(2)}</div>` : ""}${field(null, 0)}${field(null, 1)}${field(null, 2)}<div class="actions row">${btn(label("back"), "secondary")}${btn(label("next"), "primary")}</div></div>`,
  },
  {
  ...
  },
  {
    ...
    draw: ({ props, label }) => `<div class="filters">${bar(35, "k")}${rowsOf(num(props, "groups"), (g) =>
      `<div class="grp">${bar(30 + (g % 3) * 8, "k")}${rowsOf(3, (i) => `<div class="chk"><span class="cb${(i + g) % 3 === 0 ? " on" : ""}"></span>${bar(50 + i * 10, "in")}</div>`)}</div>`)}${
      flag(props, "apply") ? `<div class="actions row">${btn(label("reset"), "secondary")}${btn(label("apply"), "primary")}</div>` : ""}</div>`,
  },
  {
  ...
  },
  {
    ...
    draw: ({ props, label }) => {
      ...
      const body = (n: number) => listRows({ rows: n, leading: str(props, "leading"), trailing: str(props, "trailing") === "action" ? "none" : str(props, "trailing"), lines: 2, dividers: true }, str(props, "trailing") === "action" ? btn(label("row-action"), "secondary", "s") : "");
  ...
  },
  {
    ...
    draw: ({ props, wide }) => `<div class="grid" style="grid-template-columns:repeat(${wide ? num(props, "columns") : Math.min(2, num(props, "columns"))},1fr)">${rowsOf(num(props, "items"), (i) =>
      `<div class="card">${flag(props, "media") ? img("4/3") : ""}${bar(70 - (i % 3) * 10, "k")}${bar(45)}</div>`)}</div>`,
  },
  {
    ...
    draw: ({ props, label, intent, wide }) => {
      ...
      return `<div class="table">${flag(props, "toolbar") ? `<div class="toolbar"><div class="search sm"><span class="ico-t">${glyph("search")}</span><span class="ph-t">Search</span></div><span class="sp"></span>${
        rowsOf(3, (i) => (i === 2 ? btn(label("tool-3"), "primary", "s") : ibtn(intent(`tool-${i + 1}`), label(`tool-${i + 1}`))))}</div>` : ""}<table><thead><tr>${flag(props, "select") ? `<th class="sel"><span class="cb"></span></th>` : ""}${
        rowsOf(cols, (j) => `<th>${bar(50, "k")}</th>`)}</tr></thead><tbody>${rowsOf(num(props, "rows"), (i) => `<tr>${flag(props, "select") ? `<td class="sel"><span class="cb${i === 1 ? " on" : ""}"></span></td>` : ""}${rowsOf(cols, (j) => cell(i, j))}</tr>`)}</tbody></table>${
  ...
  },
  {
    ...
    draw: ({ props, label }) => `<div class="detail">${str(props, "media") === "none" ? "" : `${img("16/9")}${str(props, "media") === "carousel" ? `<div class="dots">${rowsOf(4, (i) => `<i class="${i === 0 ? "on" : ""}"></i>`)}</div>` : ""}`}${bar(75, "title")}<div class="metas">${
      rowsOf(num(props, "meta"), (i) => `<span class="meta-i">${icon()}${bar(60 + (i % 2) * 20, "in")}</span>`)}</div><div class="actions row">${rowsOf(num(props, "actions"), (i) => btn(label(`action-${i + 1}`), i === 0 ? "primary" : "secondary"))}</div></div>`,
  },
  // ---- social
  {
    ...
    draw: ({ props, label }) => `<div class="comments">${bar(25, "k")}${rowsOf(num(props, "comments"), (i) =>
      `<div class="comment${flag(props, "nested") && i % 2 === 1 ? " nested" : ""}">${avatar("s")}<div class="row-t">${bar(30, "k")}${bars(2, i)}</div></div>`)}${
      flag(props, "composer") ? `<div class="composer"><div class="box">${bar(40, "ph")}</div>${btn(label("post"), "primary", "s")}</div>` : ""}</div>`,
  },
  {
    ...
    draw: ({ props, label }) => `<div class="profile${flag(props, "cover") ? " covered" : ""}">${flag(props, "cover") ? img("3/1", "cover") : ""}${avatar(str(props, "avatar") === "l" ? "l" : "m")}${bar(40, "title")}${bar(28, "meta")}${
      num(props, "stats") > 0 ? `<div class="pstats">${rowsOf(num(props, "stats"), () => `<span>${bar(50, "fat")}${bar(70, "in")}</span>`)}</div>` : ""}<div class="actions row">${
      rowsOf(num(props, "actions"), (i) => btn(label(`action-${i + 1}`), i === 0 ? "primary" : "secondary"))}</div></div>`,
  },
  {
    ...
    draw: ({ props, label, intent }) => rowsOf(num(props, "count"), (p) => {
      ...
      return `<div class="fpost"><div class="fhead">${avatar("s")}<div class="row-t">${bar(35, "k")}${bar(20, "meta")}</div></div>${bars(2, p)}${
        ...
        rowsOf(num(props, "actions"), (i) => `<span class="fa">${glyph(intent(`action-${i + 1}`))} ${label(`action-${i + 1}`)}</span>`)}</div></div>`;
    }),
  },
  // ---- media
  {
  ...
  // ---- commerce
  {
    ...
    draw: ({ props, wide }) => {
      const card = (i: number) => `<div class="card product">${img(str(props, "layout") === "list" ? "1/1" : "1/1", str(props, "layout") === "list" ? "thumb-img" : "")}<div>${bar(75 - (i % 3) * 10, "k")}${
  ...
  // ---- feedback
  {
    ...
    draw: ({ props, label }) => `<div class="state">${flag(props, "media") ? img("1/1", "sm") : ""}${bar(55, "title")}${bars(2)}<div class="actions stack">${
      rowsOf(num(props, "action"), (i) => btn(label(`action-${i + 1}`), i === 0 ? "primary" : "secondary"))}</div></div>`,
  },
  {
  ...
  },
  {
    ...
    draw: ({ props, label }) => `<div class="state"><div class="glyph">✓</div>${bar(50, "title")}${bars(2)}${flag(props, "summary") ? `<div class="card">${bars(3)}</div>` : ""}<div class="actions stack">${
      rowsOf(num(props, "actions"), (i) => btn(label(`action-${i + 1}`), i === 0 ? "primary" : "secondary"))}</div></div>`,
  },
  // ---- overlay
  {
    ...
    draw: ({ props, label }) => `<div class="dialog">${bar(60, "title")}${bars(2)}${flag(props, "input") ? field(null) : ""}<div class="actions row">${btn(label("cancel"), "secondary")}${
      btn(label("confirm"), flag(props, "destructive") ? "destructive" : "primary")}</div></div>`,
  },
];
