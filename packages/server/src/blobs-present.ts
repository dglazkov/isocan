// Server-side rather than in `@isocan/core`'s protocol: only daemons ask this
// and answer it, and core's protocol module is in the web app's entry chunk,
// which paid 132 bytes for constants it never reads.
 ...
 */
export const BLOBS_PRESENT_ROUTE = "/api/projects/:id/blobs/present";
/** `BLOBS_PRESENT_ROUTE` for one canvas, spelled once so the asker and the
 *  route cannot drift apart. */
export const blobsPresentRoute = (canvasId: string): string =>
  `/api/projects/${encodeURIComponent(canvasId)}/blobs/present`;
...
export const BLOBS_PRESENT_LIMIT = 1000;
