...
export async function verifyIdToken(
  ...
  return (await verifyIdTokenClaims(token, auth, keys, now)).attestation;
}
...
export async function verifyIdTokenClaims(
  ...
  const authTime = typeof payload["auth_time"] === "number" ? payload["auth_time"] * 1000 : null;
  return {
    attestation: {
      // The grant-subject namespace, normalized once here so the door's
      // equality never has to fold anything at request time (see
      // `normalizeAttribute`).
