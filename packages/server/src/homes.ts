...
import { homesFile } from "./paths.ts";
 ...
 * theirs).
 */
export async function readHomes(home: string): Promise<HomeAssignments> {
  let parsed: unknown;
  try {
    parsed = JSON.parse(await fs.readFile(homesFile(home), "utf8")) as unknown;
  } catch {
    return {};
  }
...
export async function homesRecorded(home: string): Promise<boolean> {
  return fs.stat(homesFile(home)).then(
    () => true,
    () => false,
  );
}
 ...
 */
export async function writeHomes(home: string, rows: HomeAssignments): Promise<void> {
