...
import { applyOperation, invertOperation, resolveGroupOperation } from "@isocan/core";
...
export function groupFixture(enabled = true) {
  ...
  const client = {
    ...
    changeGroup: async (_canvasId: string, who: Actor, action: Exclude<GroupAction, { kind: "apply" }>, opId?: string): Promise<PostOpResponse> => {
      ...
      const op = resolveGroupOperation(state, { type: "group.change", action }, { actor: who, ts, opId: id });
