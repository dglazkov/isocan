import { normalizeHomeUrl, resolveActor, sourceFaceOf, visualFaceOf, type CanvasSnapshotResponse, type ItemVersion, type PostOpResponse } from "@isocan/core";
import { designComparisonActions, designDecisionScope, designDecisionIntentHash, parseDesignCompareOperation, parseDesignRespondOperation, parseDesignDecideOperation, type DesignApprovalBasis, type DesignComparison, type DesignComparisonResponse, type DesignComparisonState, type DesignDecisionInput, type DesignDecisionOperation, type DesignDecisionState, type DesignDecisionsResponse } from "@isocan/core/design-decision";
import type { DesignArtifactRef, DesignQuestionSource } from "@isocan/core/design-partner";
...
export interface DesignComparisonFilter { requestId?: string; targetItemId?: string; threadId?: string; commentId?: string; decisionKey?: string }
...
export interface DesignDecisionWritePort extends DesignDecisionReadPort {
  sendDecision(canvasId: string, operation: DesignDecisionOperation, options: { opId: string; signal?: AbortSignal }): Promise<{ status: "accepted"; receipt: PostOpResponse } | { status: "pending" | "refused"; reason: string; code?: string }>;
}
...
export interface DesignComparisonView extends DesignComparisonState {
  ...
  approvalBases: Array<{ alternativeId: string; basis: DesignApprovalBasis | null; reason?: string }>;
}
/** Accepted choice remains history even when later output or governing changes make its consistency stale. */
export interface DesignDecisionView extends DesignDecisionState { governing: GoverningDesignRead }
/** Comparison and adoption history share one projection across native text, JSON and the lazy browser surface. */
export interface DesignComparisonReadResult { comparisons: DesignComparisonView[]; decisions: DesignDecisionView[]; unavailable: DesignDecisionsResponse["unavailable"] }
/** Public publication owns stable source and operation IDs before delivery. */
export interface DesignCompareRequest { canvasId: string; threadId: string; commentId: string; opId: string; comparison: DesignComparison; signal?: AbortSignal; retry?: boolean }
/** Non-adopting outcomes preserve exact source association and distinguish native reports from canvas-human acts. */
export interface DesignRespondRequest { canvasId: string; threadId: string; commentId: string; opId: string; response: DesignComparisonResponse; signal?: AbortSignal; retry?: boolean }
/** A decision carries the basis already reviewed by its author; retry never substitutes fresh metadata. */
export interface DesignDecideRequest { canvasId: string; threadId: string; commentId: string; opId: string; decision: DesignDecisionInput; signal?: AbortSignal; retry?: boolean }
...
export interface DesignDecisionSubmission {
  status: "accepted" | "pending" | "refused"; canvasId: string; threadId: string; commentId: string; payloadId: string;
  submittedOpId: string; opId: string | null; confirmedBy?: "receipt" | "snapshot"; seq?: number; reason?: string;
...
const sameBinding = (a: DesignGoverningBinding, b: DesignGoverningBinding) => a.atItemId === b.atItemId && a.explicitNone === b.explicitNone && (a.artifact === null ? b.artifact === null : b.artifact !== null && sameDesignArtifact(a.artifact, b.artifact));
const matches = (comparison: DesignComparison, source: DesignQuestionSource, filter: DesignComparisonFilter, targetItemId?: string) => (!filter.requestId || comparison.requestId === filter.requestId) && (!filter.targetItemId || filter.targetItemId === (targetItemId ?? comparison.target.itemId) || comparison.target.itemId === null && comparison.alternatives.some(one => one.artifact.itemId === filter.targetItemId)) && (!filter.threadId || source.threadId === filter.threadId) && (!filter.commentId || source.commentId === filter.commentId) && (!filter.decisionKey || comparison.decisionKey === filter.decisionKey);
const binding = (atItemId: string | null, governing: GoverningDesignRead): DesignGoverningBinding => ({ atItemId, artifact: governing.artifact, explicitNone: governing.exempt });

/** Resolve exact comparison versions and captured approval metadata through the same governing policy as other design reads. */
export async function readDesignComparisons(io: DesignDecisionReadPort, options: { canvasId: string; filter?: DesignComparisonFilter; signal?: AbortSignal }): Promise<DesignComparisonReadResult> {
  ...
  const reads = new Map<string, Promise<GoverningDesignRead>>();
  const at = (itemId: string | null) => {
    const key = itemId ?? ""; let promise = reads.get(key);
    if (!promise) { promise = readGoverningDesign(io, { canvasId, canvas: snapshot.canvas, project: snapshot.project, home, ...(itemId ? { atId: itemId } : {}), ...(signal ? { signal } : {}) }); reads.set(key, promise); }
    return promise;
  };
  const comparisons: DesignComparisonView[] = [];
  for (const state of records.comparisons.filter(one => matches(one.comparison, one.source, filter))) {
    ...
    const approvalBases: DesignComparisonView["approvalBases"] = [];
    for (const alternative of comparison.alternatives) {
      ...
      if (!item || !version) { approvalBases.push({ alternativeId: alternative.id, basis: null, reason: "The adoption target is unavailable." }); continue; }
      const selectedGoverning = await at(item.id);
      if (selectedGoverning.status === "unavailable") { approvalBases.push({ alternativeId: alternative.id, basis: null, reason: selectedGoverning.reason }); continue; }
  ...
  const decisions: DesignDecisionView[] = [];
...
function identifiers(request: { canvasId: string; threadId: string; commentId: string; opId: string }): void {
...
export function prepareDesignDecision(request: DesignDecideRequest): DesignDecideRequest {
  identifiers(request);
...
async function preflight(io: DesignDecisionWritePort, request: DesignCompareRequest | DesignRespondRequest | DesignDecideRequest, op: DesignDecisionOperation): Promise<void> {
  const bindings: DesignGoverningBinding[] = [];
  ...
  for (const expected of bindings) {
    const key = JSON.stringify(expected); if (checked.has(key)) continue; checked.add(key);
...
async function submit(io: DesignDecisionWritePort, request: DesignCompareRequest | DesignRespondRequest | DesignDecideRequest, op: DesignDecisionOperation): Promise<DesignDecisionSubmission> {
  identifiers(request);
  const payloadId = op.type === "design.compare" ? op.comparison.id : op.type === "design.respond" ? op.response.id : op.decision.id;
  const result: DesignDecisionSubmission = { status: "pending", canvasId: request.canvasId, threadId: request.threadId, commentId: request.commentId, payloadId, submittedOpId: request.opId, opId: null };
  // Only a fresh intent preflights current sources. An uncertain exact retry must reach
  // canonical receipt lookup even after those sources move or access becomes unavailable.
  if (!request.retry) {
    try { await preflight(io, request, op); }
    catch (error) { return { ...result, status: "refused", reason: message(error) }; }
  }
  let delivered: Awaited<ReturnType<DesignDecisionWritePort["sendDecision"]>>;
  try { delivered = await io.sendDecision(request.canvasId, op, { opId: request.opId, ...(request.signal ? { signal: request.signal } : {}) }); }
  catch (error) { delivered = { status: questionnaireFailureStatus(error), reason: message(error), ...(error && typeof error === "object" && "code" in error && typeof error.code === "string" ? { code: error.code } : {}) }; }
  ...
  try { const current = await io.snapshot(request.canvasId, request.signal); if (current.project.id === request.canvasId) snapshot = current; } catch { /* A matching receipt may still prove acceptance. */ }
  ...
  let accepted: DesignDecisionSubmission | undefined;
  if (delivered.status === "accepted") {
    ...
    if (envelope?.canvasId === request.canvasId && sameAuthor(envelope.actor.id)) {
      try { if (await designDecisionIntentHash(envelope.op as DesignDecisionOperation, envelope.actor.id) === await designDecisionIntentHash(op, envelope.actor.id)) accepted = { ...result, status: "accepted", opId: envelope.id, confirmedBy: "receipt", seq: delivered.receipt.seq }; } catch { /* Unrelated canonical receipt is not this intent. */ }
    }
  }
...
export function publishDesignComparison(io: DesignDecisionWritePort, request: DesignCompareRequest): Promise<DesignDecisionSubmission> {
...
export function respondDesignComparison(io: DesignDecisionWritePort, request: DesignRespondRequest): Promise<DesignDecisionSubmission> {
...
export async function submitDesignDecision(io: DesignDecisionWritePort, request: DesignDecideRequest): Promise<DesignDecisionSubmission> {
...
export interface DesignComparisonReference { artifact: DesignArtifactRef; version: ItemVersion; title: string; face: "source" | "visual"; bytes: Uint8Array }
/** Open a named option under an exact source, never an unscoped option ID or a latest-version fallback. */
export async function readDesignComparisonReference(io: DesignDecisionReadPort, request: { canvasId: string; source: DesignQuestionSource; optionId: string; face?: "source" | "visual"; signal?: AbortSignal }): Promise<DesignComparisonReference> {
  ...
  const version = (local ? retained?.version : undefined) ?? snapshot.canvas.items[artifact.itemId]?.versions.find(one => one.id === artifact.versionId);
