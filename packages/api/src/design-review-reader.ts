...
import { type CanvasDesignAudit, type SourceDesignAudit } from "./design-audit-reader.ts";
import { readGoverningDesign } from "./design-governing.ts";
import { DESIGN_REVIEW_PROPERTY, parseDesignReviewRun, parseDesignVerifierOffer, designReviewSourceAudit, type DesignReviewRun, type DesignVerifierOffer } from "./design-review-contract.ts";
...
export interface DesignReviewReadings { source: "passed" | "failed" | "unsupported" | "unavailable"; task: "passed" | "failed" | "unavailable"; craft: "passed" | "failed" | "unavailable"; missingObligationIds: string[]; sourceAudit: CanvasDesignAudit | SourceDesignAudit | null; limits: string[] }
...
export interface DesignReviewView {
  ref: DesignArtifactRef; run: DesignReviewRun; author: Actor;
  status: "current" | "stale" | "unavailable"; reasons: string[];
  remainingRepairs: number | null; ready: boolean; readings: DesignReviewReadings;
  nextAction: "record" | "repair" | "finish" | "review-inputs";
  allowedActions: { record: boolean; beginRepair: boolean; finish: boolean; handoff: boolean };
  passes: Array<{ id: string; reservedBy: Actor | null; recordedBy: Actor | null; repairs: DesignRepairsResponse["repairs"] }>;
  versions: Array<{ ref: DesignArtifactRef; author: Actor; seq: number }>;
  handoffs: Array<{ opId: string; threadId: string; commentId: string; author: Actor; verifierActorId: string; status: "requested"; standing: "active" | "removed" }>;
}
/** Eligibility combines this exact expiring report with actual liveness and existing wake policy. */
export interface DesignVerifierView { ref: DesignArtifactRef; offer: DesignVerifierOffer; author: Actor; eligible: boolean; reasons: string[] }
...
function versions(op: Operation): Array<{ itemId: string; version: Version["version"] }> {
  const adds = created(op); if (adds.length) return adds;
...
export function designReviewReadings(run: DesignReviewRun): DesignReviewReadings {
  ...
  const limits: string[] = [];
  for (const row of rows) if (row.status === "unavailable") limits.push(row.reason); else {
  ...
  const reading = (kind: "browser-task" | "craft"): "passed" | "failed" | "unavailable" => {
...
export async function readDesignReviewReference(io: DesignReviewReadPort, options: { canvasId: string; artifact: DesignArtifactRef; signal?: AbortSignal }): Promise<{ artifact: DesignArtifactRef; version: ItemVersion; bytes: Uint8Array; local: boolean }> {
  ...
  if (!version || version.blobHash !== artifact.blobHash) {
    if (local) {
      ...
      if (owner) { const retained = await readDesignRequestReference(io, { canvasId, requestId: owner.brief.requestId, artifact, ...(signal ? { signal } : {}) }); return { artifact, version: retained.version, bytes: retained.bytes, local }; }
    }
...
export async function readDesignReviews(io: DesignReviewReadPort, options: { canvasId: string; requestId?: string; runId?: string; signal?: AbortSignal; now?: number }): Promise<DesignReviewReadResult> {
  ...
  // Original creation metadata survives removal and blob collection. Mutable properties cannot relocate history.
  type Binding = { requestId: string; runId: string };
  const bindings = new Map<string, Binding[]>();
  const addBinding = (itemId: string, binding: Binding) => { const list = bindings.get(itemId) ?? []; if (!list.some(one => same(one, binding))) bindings.set(itemId, [...list, binding]); };
  for (const entry of log) if (!entry.cause) for (const one of created(entry.envelope.op)) if (ids.has(one.itemId)) {
  ...
  const inScope = (itemId: string) => !bindings.has(itemId) || bindings.get(itemId)!.some(one => (!options.requestId || one.requestId === options.requestId) && (!options.runId || one.runId === options.runId));
  ...
  const records = new Map<string, Version>(), conflicts: DesignReviewReadResult["unavailable"] = [];
  for (const entry of [...log].sort((a, b) => a.seq - b.seq)) if (!entry.cause) for (const one of versions(entry.envelope.op)) if (ids.has(one.itemId)) {
    const key = `${one.itemId}/${one.version.id}`, previous = records.get(key);
    const author = "createdBy" in one.version ? one.version.createdBy as Actor : entry.envelope.actor;
  ...
  const unavailable: DesignReviewReadResult["unavailable"] = [...conflicts], runs: DesignReviewView[] = [], offers: DesignVerifierView[] = [];
  ...
  const parsed = new Map<string, Array<Version & { ref: DesignArtifactRef; report: DesignReviewRun | DesignVerifierOffer }>>();
  for (const row of records.values()) {
    ...
    } catch (error) { unavailable.push({ itemId: row.itemId, reason: `Review history ${row.version.id}: ${reason(error)}` }); }
  }
  // Readable legacy documents can establish scope; fully unidentified history remains explicitly unavailable.
  ...
  for (const [itemId, history] of parsed) {
    ...
    if (latest.report.kind === "verifier-offer") { const offer = latest.report; if ((!options.requestId || offer.requestId === options.requestId) && (!options.runId || offer.runId === options.runId)) offers.push({ ref: latest.ref, offer, author: latest.author, eligible: false, reasons: currentId === latest.version.id ? [] : ["The offer is no longer a current artifact."] }); continue; }
    ...
    const prior = history.filter((one): one is typeof one & { report: DesignReviewRun } => one.report.kind === "review-run");
    ...
    for (const p of run.passes) {
      ...
      if (reservations.some(one => { const old = one.report.passes.find(old => old.id === p.id)!; return old.kind !== p.kind || old.reservationVersionId !== p.reservationVersionId || old.sessionId !== p.sessionId || old.reservedAt !== p.reservedAt; })) reasons.push(`Reservation ${p.id} changed its identity.`);
      if (firstRecorded && reservations.some(one => { const old = one.report.passes.find(old => old.id === p.id)!; return old.record !== null && !same(old, firstRecorded); })) reasons.push(`Recorded pass ${p.id} was rewritten; its original observer cannot be credited for changed evidence.`);
      ...
      if (original && reservations.some(one => { const old = one.report.passes.find(old => old.id === p.id)!; return !old.record && !same(old.output, original.output); })) reasons.push(`Pending pass ${p.id} changed its captured output.`);
    ...
    const handoffs: DesignReviewView["handoffs"] = log.flatMap(entry => {
      ...
      const line = op.comment.body.split("\n").find(line => line.startsWith("Exact run: "));
      if (!line) return [];
      try { const ref = parseDesignArtifactRef(JSON.parse(line.slice("Exact run: ".length))); if (ref.itemId !== itemId || ref.canvasId !== canvasId || !history.some(one => sameDesignArtifact(one.ref, ref))) return []; }
      catch { return []; }
      const present = snapshot.canvas.threads[op.threadId]?.comments.some(c => c.id === op.comment.id);
      return [{ opId: entry.envelope.id, threadId: op.threadId, commentId: op.comment.id, author: entry.envelope.actor, verifierActorId: op.comment.mentions[0]!, status: "requested" as const, standing: present ? "active" as const : "removed" as const }];
    });
    runs.push({ ref: latest.ref, run, author: latest.author, status: reasons.length ? incompleteHistory ? "unavailable" : "stale" : "current", reasons: [...new Set(reasons)], remainingRepairs, ready, readings, nextAction: !active ? "review-inputs" : !pass.record ? "record" : ready || !allowedActions.beginRepair ? "finish" : "repair", allowedActions, passes: run.passes.map(p => { const first = prior.find(one => one.report.passes.some(old => old.id === p.id)); const recorded = prior.find(one => one.report.passes.some(old => old.id === p.id && old.record)); return { id: p.id, reservedBy: first?.author ?? null, recordedBy: recorded?.author ?? null, repairs: repairs.repairs.filter(one => one.repair.review?.runId === run.id && one.repair.review.passId === p.id && one.repair.review.run.itemId === itemId) }; }), versions: history.map(one => ({ ref: one.ref, author: one.author, seq: one.seq })), handoffs });
  }
  let sessions: PresenceSession[] = [], answering: RcAnsweringResponse | null = null, actors: Awaited<ReturnType<DesignReviewReadPort["decisionActors"]>> = { actors: [] };
  try { [sessions, answering, actors] = await Promise.all([io.sessions(canvasId, signal), io.answering(canvasId, signal), io.decisionActors(canvasId, signal)]); } catch { /* Offers remain visible but ineligible. */ }
  for (const row of offers) {
    ...
    if (!io.actorId || !answering?.actorIds.some(id => resolveActor(snapshot.joined, id) === resolveActor(snapshot.joined, row.author.id)) || !policy || !mayWake(policy, io.actorId, snapshot.joined, undefined, now)) row.reasons.push("The current actor has no confirmed authorization to wake this verifier.");
