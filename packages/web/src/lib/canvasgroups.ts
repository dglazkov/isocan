import type { Actor, GroupAction, Item } from "@isocan/core";
...
export async function createCanvasGroup(canvasId: string, actor: Actor, title: string, brief: string, itemIds: string[], at: { x: number; y: number }): Promise<string> {
  ...
  const action = itemIds.length ? groupWrapAction(canvas, creation, itemIds) : { kind: "create" as const, group: creation, containerId: useUiStore.getState().activeGroupId };
