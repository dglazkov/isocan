...
import { registerModule, type UnderlayFacts, type WebModule } from "@isocan/core";
 ...
 * record so `isocan context`'s rows and the JSON Canvas edges agree with what
 * is drawn. Remove a line here (and the twin in `packages/cli/src/modules.ts`)
 * and the module is gone from this surface — its items stay, as files.
 *
 ...
 */
export type ShellModule = WebModule<ComponentType<UnderlayFacts>>;

export const MODULES: readonly ShellModule[] = [mindmapWeb];
