...
import type { Actor, Grant } from "@isocan/core";
 ...
 *    arrival; it does not expel anybody, and the dialog says so rather than
 *    implying a sweep that is phase 9's.
 ...
 *
 * **There is no "who" field, on purpose.** The design's `email:` subject needs
 * an attester, phase 9 owns attesters, and `POST …/grants` refuses `email:`
 * today by naming that phase. A field that wrote a row admitting nobody would
 * be a dialog that lies, so the deferral is stated in one line of prose
 * instead — the same answer the API gives, in the place a person would ask
 * the question.
 *
...
export function ShareDialog({ actor, onClose }: { actor: Actor; onClose: () => void }) {
  ...
  async function toggleLink(): Promise<void> {
    ...
    try {
      ...
      if (link) await revokeGrant(projectId, link.id);
      else await createGrant(projectId, LINK);
  ...
  return (
    ...
    >
        ...
        <span className="share-link-text">
          ...
          <span className="share-link-note">
            {grants === null
              ? "asking the door…"
              : linkOn
                ? "anyone who has the address can open this canvas"
                : // Say what revocation actually did. Implying an expulsion
                  // that will not happen until phase 9's sweep would be the
                  // dialog making a promise the door has not made.
                  "new arrivals are turned away. People already on this canvas keep their access."}
          </span>
        </span>
      </button>
      ...
      <div className="share-deferred">
        Inviting one person by email is not built yet — the link is how you share today.
      </div>
