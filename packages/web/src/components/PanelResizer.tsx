...
export function PanelResizer() {
  ...
  function onPointerDown(e: React.PointerEvent) {
    ...
    function onMove(ev: PointerEvent) {
      useUiStore.getState().setPanelWidth(startWidth + (ev.clientX - startX));
    }
  ...
  return (
    <div
      ...
      onDoubleClick={() => useUiStore.getState().setPanelWidth(PANEL_MIN_WIDTH)}
      ...
      aria-valuemax={maxPanelWidth(typeof window === "undefined" ? 1280 : window.innerWidth)}
      tabIndex={0}
      onKeyDown={(e) => {
        const step = e.shiftKey ? 64 : 16;
        if (e.key === "ArrowLeft") {
          e.preventDefault();
          useUiStore.getState().setPanelWidth(width - step);
        } else if (e.key === "ArrowRight") {
          e.preventDefault();
          useUiStore.getState().setPanelWidth(width + step);
        } else if (e.key === "Home") {
          e.preventDefault();
          useUiStore.getState().setPanelWidth(PANEL_MIN_WIDTH);
        }
      }}
    />
  );
