...
export function DesignTaskCard({ canvasId, actor, row, canEdit, checking = false, onChanged }: { canvasId: string; actor: Actor | null; row: DesignRequestView; canEdit: boolean; checking?: boolean; onChanged: () => void }) {
  ...
  return <article className="design-task-card" data-design-request={brief.requestId}>
    ...
    {actor && (row.comparisons.length > 0 || row.decisionHistory.length > 0 || hasSavedComparisonPending(canvasId, actor.id, brief.requestId)) && <section className="design-task-comparisons"><DesignComparisonButton canvasId={canvasId} actor={actor} filter={{ requestId: brief.requestId }}>{!row.comparisons.length && !row.decisionHistory.length ? "Recover saved choice" : row.effectiveDecisions.length ? "Review design choices" : "Compare options"}</DesignComparisonButton>{row.effectiveDecisions.map((saved) => <p key={saved.decision.input.id}>{saved.decision.comparison.alternatives.find((one) => one.id === saved.decision.input.chosenAlternativeId)?.title} · {saved.decision.input.authority.kind === "human-choice" ? `chosen by ${saved.author.name}` : `decision by ${saved.author.name}`}{saved.decision.input.authority.kind === "human-choice" && saved.decision.input.authority.reason ? `: ${saved.decision.input.authority.reason}` : saved.decision.input.authority.kind !== "human-choice" ? `: ${saved.decision.input.authority.rationale}` : ""}</p>)}</section>}
