  ...
  it("is written down once in the source, and imported everywhere else", async () => {
    ...
    const walk = async (dir: string): Promise<void> => {
      for (const entry of await fs.readdir(dir, { withFileTypes: true })) {
        ...
        // SKILL.md verbatim, generated by scripts/rc-skill.mjs for hosts with
        // no disk; packages/rc/test/skill.test.ts holds it equal to the file.
        if (full.endsWith(path.join("rc", "src", "skill.ts"))) continue;
